...
4. Verify the traffic split (8:2 ratio)

> This is a **manual canary** pattern where traffic is split based on replica counts.
...
The Service selects on `app=webapp` — both deployments need this label on their pod template. Use `version` to differentiate in `selector.matchLabels`.
</details>
